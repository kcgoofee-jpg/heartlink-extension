...
- **gen / read** 属于上一条回复：你等它生成、读它时的心率。模型据此判断上一段写得怎么样。
- **write / send** 属于这一条消息：你打字、按下发送时的心率。
- 页面切走或长时间没操作的时段（away）不计入。
- 平静心率（基线）默认按本场安静时段自动算，也可以手动记。
- 悬浮窗的健康设备页实时画出本轮走势：底色是相位（看生成细分为等首字 / 思考 / 正文），斜线是离开的时段，虚线是平静心率；胶囊上的 +N% 也是和平静心率比。
