// heartlink v0.24.1 — SillyTavern 扩展：心率注入 + 触觉输出（Tavern Bio-Context 参考实现）。构建产物。
...
(function heartlinkRuntime() {
  ...
  const MILESTONE = { name: '里程碑 3', notes: ['心跳节奏跟着相位微调：生成中更从容，打字更专注，判定出峰值那一拍会跳大一点', '模型读完上一轮的判断会在胶囊或面板上闪一下，看得出"传回来了"', '胶囊多一圈细环，显示这轮攒的数据离下次发出去还差多少', '平静心率旁边多一个呼吸的小环，不用看数字也能感觉出偏离', '离开时：切走页面才整体变灰，光是发一会儿呆不算，只是心跳轻一点'] };
  const CONFIG = {
...
.mnotes{margin:4px 0 2px;padding-left:16px;font:12px/18px var(--font);color:var(--muted)}.mnotes li{margin:2px 0}
...
<div class="pill" part="pill" title="heartlink">
  ...
  function settingsPaneHtml() {
    ...
    return '<div class="mgroup"><div class="mgh">离开与阅读</div>'
      ...
      + '</div>'
      + (labUp ? '<div class="mgroup"><div class="mgh">设备实验室</div>'
        + '<div class="mrow"><div class="ml"><span class="mk">本机的设备实验室</span><span class="msub">虚拟玩具、假手环、字节日志；在小窗口里打开</span></div><div class="mc"><button class="tune" data-act="lab">打开 ›</button></div></div></div>' : '')
      + '<div class="mgroup"><div class="mgh">这一版 · v' + VERSION + ' · ' + esc(MILESTONE.name) + '</div><ul class="mnotes">' + MILESTONE.notes.map((n) => '<li>' + esc(n) + '</li>').join('') + '</ul></div>'
